...
export class OfflineHubService {
  ...
  static async downloadTrackToHub(song: {
    ...
    try {
      ...
      // Evict old space if quota is full
      await this.ensureSpaceForNewTrack(10 * 1024 * 1024); // ~10MB estimation
  ...
  static async ensureSpaceForNewTrack(requiredBytes: number) {
    try {
      ...
      const currentBytes = store.getTotalUsedBytes();
